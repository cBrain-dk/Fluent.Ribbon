...
namespace FluentTest
{
    ...
    public partial class TestContent : UserControl
    {
        ...
        private int TabGeneratorCount { get; set; }

        private RibbonTabItem CurrentAutoTabItem { get; set; }

        private void ShowGeneratedContextualRibbonTabCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            var contextGroup = this.ribbon.ContextualGroups.Last();
            this.TabGeneratorCount++;

            var item = new RibbonTabItem()
            {
                Header = $"AutoGenerated Tab no.: {this.TabGeneratorCount}",
                Group = contextGroup
            };

            item.Groups.Add(this.GetGroupWithButton("Group 1"));
            item.Groups.Add(this.GetGroupWithButton("Group 2"));

            this.CurrentAutoTabItem = item;

            this.ribbon.Tabs.Add(item);
            contextGroup.Visibility = Visibility.Visible;
        }

        private void ShowGeneratedContextualRibbonTabCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            this.CurrentAutoTabItem.Group = null;
            this.ribbon.Tabs.Remove(this.CurrentAutoTabItem);
            this.CurrentAutoTabItem = null;

            var contextGroup = this.ribbon.ContextualGroups.Last();
            contextGroup.Visibility = Visibility.Collapsed;
        }

        private RibbonGroupBox GetGroupWithButton(string name)
        {
        ...
        private void DelayedSelectTabButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                this.delayTimer = new DispatcherTimer(TimeSpan.FromSeconds(5), DispatcherPriority.Normal, TimerHandler, this.Dispatcher);
            }
            else if (this.delayTimer.IsEnabled)
            {
                this.delayTimer.Stop();
            }
